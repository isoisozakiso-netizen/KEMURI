...
async function handleSharePage(request, env, shopId){
  ...
  const title = (meta.name ? `${meta.name} - 一服` : "一服 — 姫路グルメ 喫煙席検索");
  const desc = meta.name
    ? `姫路で煙草が吸える${meta.genre?`「${meta.genre}」`:""}「${meta.name}」。一服で喫煙環境をチェック。`
    : "姫路の喫煙席があるお店を検索 — 一服";
  const ogImage = `${url.origin}/icon.png`;
  const pageUrl = `${url.origin}/s/${encodeURIComponent(shopId)}`;

  // index.html を取得
  let html;
  try{
    const assetReq = new Request(`${url.origin}/index.html`, request);
